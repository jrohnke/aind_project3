...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # go through all possible actions, create an a-node and check if the preconditions are fulfilled in previous s-level
        # add parent nodes from previous s-level and add node to the new a-level
        self.a_levels.append(set())
        for action in self.all_actions:
            node = PgNode_a(action)
            if node.precond_s_nodes().issubset(self.s_levels[level]):
                for precond in self.s_levels[level]:
                    if precond in node.precond_s_nodes():
                        node.parents.add(precond)
    ...
    def add_literal_level(self, level):
        ...
        # go through all effects of the previous level actions, add children and parents connection and add nodes to new s-level
        self.s_levels.append(set())
        for action in self.a_levels[level-1]:
            for eff_s_node in action.effect_s_nodes():
                eff_s_node.parents.add(action)
                action.children.add(eff_s_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        if (set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)) or \
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        return (set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)) or \
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for parent_a1 in node_a1.parents:
            for parent_a2 in node_a2.parents:
                if parent_a1 in parent_a2.mutex or parent_a2 in parent_a1.mutex:
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_s1 in node_s1.parents:
            for parent_s2 in node_s2.parents:
                if node_s1 in parent_s2.effnodes and node_s2 in parent_s1.effnodes:
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        # loop through all the s-levels, look at the literals and remove from goalstates if there is a match
        logging.debug("New levelsum evaluation!")
        logging.debug("Goals: {}".format(goals))
        for level in range(len(self.s_levels)):
            level_states = set([s.literal for s in self.s_levels[level]])
            reached = goals & level_states
            logging.debug("level: {}, reached: {}".format(level_states, reached))
            if reached:
                level_sum += level * len(reached)
                logging.debug("level_sum: {}".format(level_sum))
                goals = goals.difference(level_states)
                logging.debug("Goals: {}".format(goals))
            if not goals:
                return level_sum

        logging.debug("Did not find all the goals, returning inf")
        return float("inf")
